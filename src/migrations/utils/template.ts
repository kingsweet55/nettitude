...
export const up = async () => {
   ...
   const keypair_item = {
      "publicKey": "mmsj322f231hp1pw_test",
      "envPrivateKey": "1j1j1j1_test"
   }

   await keypairs.insertOne(keypair_item)
      .then(result => console.log(`Successfully inserted "keypairs" item with _id: ${result.insertedId}`))
      .catch(err => console.error(`Failed to insert item: ${err}`));
   ...
   await db.createCollection("attachment", {
      validator: {
         $jsonSchema: {
            bsonType: "object",
            required: [""],
